...
int temp_uc () {
  ...
  vector<vector<uint16_t>> TempofUC;
  ...
  for (int i=0; i<NofFiles; i++) {
    ...
    size_t result;
    char filename[40];
    snprintf(filename, sizeof(filename), "TVAC_UC/file_UC_%d.txt", i);
  ...
  for(int i=0; i<NofSIAB; i++){
    for(int j=0; j<NofFiles*NofSamples; j++){
      if(TempofUC[j][i] > 100){
        Temperature[i][j]=(TempofUC[j-2][i] + TempofUC[j+2][i])/2;
      }
      else{
        Temperature[i][j]=TempofUC[j][i];
      }
