...
void AD7173Class::sync(uint8_t ADC_ID)
{
  for (int i=0; i<4; i++) {
    if (ADC_ID == i+1) {
...
int AD7173Class::set_register(adc7173_register_t reg, byte *value, int value_len) {
	...
	delayMicroseconds(READ_WRITE_DELAY);
	/* return error code */
	return 0;
}
...
int AD7173Class::set_offset_config(adc7173_register_t offset, uint32_t offset_value) {
	...
	offset_value += 0x800000; // was 8388608, hex is more clear
	/* prepare the configuration value */
...
bool AD7173Class::init_ADC(uint8_t ADC_ID) {
  
  AD7173.sync(ADC_ID);
  Serial.println("ADC chip select goes low.");

  AD7173.reset();
  Serial.println("ADC is reset and ready.");

  if (AD7173.is_valid_id())
  {
    Serial.print("HK ADC Number ");
    Serial.print(ADC_ID);
    Serial.println(" is valid.");

    Serial.println("Enabling ADC Channels ...");
    AD7173.set_channel_config(CH0,  true, SETUP0, AIN1,  REF_NEG);
    AD7173.set_channel_config(CH1,  true, SETUP0, AIN2,  REF_NEG);
    AD7173.set_channel_config(CH2,  true, SETUP0, AIN3,  REF_NEG);
    AD7173.set_channel_config(CH3,  true, SETUP0, AIN4,  REF_NEG);
    AD7173.set_channel_config(CH4,  true, SETUP0, AIN5,  REF_NEG);
    AD7173.set_channel_config(CH5,  true, SETUP0, AIN6,  REF_NEG);
    AD7173.set_channel_config(CH6,  true, SETUP0, AIN7,  REF_NEG);
    AD7173.set_channel_config(CH7,  true, SETUP0, AIN8,  REF_NEG);
    AD7173.set_channel_config(CH8,  true, SETUP0, AIN9,  REF_NEG);
    AD7173.set_channel_config(CH9,  true, SETUP0, AIN10, REF_NEG);
    AD7173.set_channel_config(CH10, true, SETUP0, AIN11, REF_NEG);
    AD7173.set_channel_config(CH11, true, SETUP0, AIN12, REF_NEG);
    AD7173.set_channel_config(CH12, true, SETUP0, AIN13, REF_NEG);
    AD7173.set_channel_config(CH13, true, SETUP0, AIN14, REF_NEG);
    AD7173.set_channel_config(CH14, true, SETUP0, AIN15, REF_NEG);
    AD7173.set_channel_config(CH15, true, SETUP0, AIN16, REF_NEG);

    AD7173.set_setup_config(SETUP0, UNIPOLAR, AIN_BUF_DISABLE, REF_EXT); // REF_PWR, REF_INT, REF_AIN
